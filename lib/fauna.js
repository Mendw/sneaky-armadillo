...
export async function user_from_client_id(provider_id, client_id) {
    return await client.query(
        q.Let({
            ...
            error: 'Bad account: referenced user_ref not found'
        })), {
...
export async function getSession(session_ref) {
    return await client.query(
        q.Let({
            session_ref: q.Ref(q.Collection('sessions'), session_ref)
        }, q.If(q.Exists(q.Var('session_ref')), {
            data: q.Select(['data'], q.Get(q.Var('session_ref')))
        }, {
...
export async function updateSession(session_ref, data) {
    return await client.query(
        q.Let({
            session_ref: q.Ref(q.Collection('sessions'), session_ref)
        }, q.If(q.Exists(q.Var('session_ref')), {
            data: q.Select(['data'], q.Update(q.Get(q.Var('session_ref')), data))
        }, {
...
export async function deleteSession(session_ref) {
    return await client.query(
        q.Let({
            session_ref: q.Ref(q.Collection('sessions'), session_ref)
        }, q.If(q.Exists(q.Var('session_ref')), {
            data: q.Select(['data'], q.Delete(q.Get(q.Var('session_ref'))))
        }, {
